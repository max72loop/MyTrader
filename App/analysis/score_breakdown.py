...
fig = go.Figure()
fig.add_trace(go.Scatterpolar(r=values, theta=categories, fill='toself', name=row.get('ticker','TICKER'), fillcolor='rgba(102,126,234,0.3)', line=dict(color='#667eea', width=3)))
fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[-3,3], gridcolor='lightgray')), showlegend=False, height=500, paper_bgcolor='white', font=dict(size=12))
return fig