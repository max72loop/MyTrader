...
with c1: x = st.selectbox("📊 Indicateur X (horizontal)", feature_cols, index=0)
with c2: y = st.selectbox("📊 Indicateur Y (vertical)", feature_cols, index=min(1, len(feature_cols)-1))
with c3: color_by = st.selectbox("🎨 Colorer par", ["Aucun"] + [c for c in ["sector","region","style"] if c in df_feats.columns])
filt = apply_filters(df_feats, sectors_sel, regions_sel)
if filt.empty:
st.warning("Aucune donnée après application des filtres")
return
plot_cols = [x, y]
if color_by != "Aucun" and color_by in filt.columns: plot_cols.append(color_by)
for c in ("ticker","name"):
if c in filt.columns: plot_cols.append(c)
...
return
fig = px.scatter(plot_df, x=x, y=y, color=(color_by if color_by!="Aucun" else None), hover_data=[c for c in ["ticker","name"] if c in plot_df.columns], title=f"Relation entre {x} et {y}", labels={x:x, y:y})
fig.update_traces(marker=dict(size=10, line=dict(width=1, color='white')))
fig.update_layout(height=600, plot_bgcolor='white', paper_bgcolor='white', font=dict(size=12), xaxis=dict(gridcolor='lightgray', showgrid=True), yaxis=dict(gridcolor='lightgray', showgrid=True))
st.plotly_chart(fig, use_container_width=True)
if len(plot_df) > 2:
corr = plot_df[x].corr(plot_df[y])
c1,c2,c3 = st.columns(3)
with c1: st.metric("Nombre de points", len(plot_df))
with c2: st.metric("Corrélation", f"{corr:.3f}")
with c3:
